...
class ProductosController {
    ...
    static async agregar(req, res){
        ...
        try {
            ...
            let data = await ProductosModel.agregar(req.body)
        
            data = await ProductosModel.consultarPorId(data[0]);
            if (data.length == 0){
    ...
    static async editar(req, res){
        ...
        try {
            ...
            let data = await ProductosModel.editar(req.params.id,req.body)
            
            data = await ProductosModel.consultarPorId(req.params.id);
            if (data.length == 0){
                res.status(404).send({code: 404, message:'not found'});
            }
            res.send(data[0]);
